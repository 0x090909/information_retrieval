...
Lo scopo finale di un Sistema di Information Retrieval (IRS) \`e quello di reperire documenti
 ...
 importanti, utili o necessarie per la risoluzione del problema o lo svolgimento del compito};
  ...
  Ci\`o che descrive i documenti e ci\`o che descrive le interrogazioni deve essere confrontabile, infatti nei programmi di indicizzaizone e di reperimento si usa uno schema, che si deve tenere in considerazione per entrambi i casi.
%che deve essere uguale in entrambi i casi.
  
L'indicizzazione \`e un trade-off tra il  miglioramento della rapprerentazione del
contenuto informativo dei documenti (efficacia) e la gestione degli indici (efficienza), in questa relazione tratter\`a il problema dell'eficacia.
...
Un sistema IR si puo' basare su vari modelli e metodi di rappresentazione delle informazioni e di reperimento.
Come gia' detto, in questa sezione si presenteranno quelli scelti per implementare il motore di ricerca utilizzato negli esperimenti.
...
\begin{figure}%
    ...
    \caption{Indice inverso e eliminazione delle stopword, per dare un'idea.}%
\end{figure}
...
Si \`e deciso di limitare il numero di risultati proposti a 1000 articoli inquanto si \`e pensato che, 
anche se un articolo rilevante fosse in posizione dopo la millesima, non verrebbe visto dall'utente
che probabilmente controlla solo le prime pagine.
...
Si pu\`o vederne un esempio in Figura 3.
